...
public class JooqLocking {
    ...
    public static void selectWithLockingForUpdate(int vehicleId, BigDecimal price) {
    ...
    public static void selectWithLockingForShare(int vehicleId, BigDecimal price){
        DSLContext context = DSL.using(new CustomConnectionProvider(), SQLDialect.MYSQL);
        Table vehicleTable = table("vehicle");
        Field<BigDecimal> vehicleCurrentPrice = field("vehicle_curr_price", BigDecimal.class);
        Condition vehicleCondition = condition("vehicle_id = ?", vehicleId);
