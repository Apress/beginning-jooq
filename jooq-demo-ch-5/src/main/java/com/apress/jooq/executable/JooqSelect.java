...
public class JooqSelect {
    ...
    public static void navigateRecord() throws SQLException {
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/edens_car?user=root&password=admin")) {
            DSLContext context = DSL.using(connection, SQLDialect.MYSQL);
            VehicleRecord vehicleRecord = context.fetchOne(VEHICLE, VEHICLE.VEHICLE_ID.eq(val(7L)));
            VehicleManufacturerRecord vehicleManufacturerRecord = vehicleRecord.fetchParent(Keys.VEH_MANUFACTURER_ID);
            VehicleModelRecord vehicleModelRecord = vehicleRecord.fetchParent(Keys.VEH_MODEL_ID);
    ...
    public static void selectIntoRecord() throws SQLException {
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/edens_car?user=root&password=admin")) {
            DSLContext context = DSL.using(connection, SQLDialect.MYSQL);
            Result<VehicleRecord> vehicles = context.fetch(VEHICLE, VEHICLE.VEHICLE_MANUFACTURER.eq(val(2L)));
        }
    }
    ...
    public static void selectWithGeneratedCode() throws SQLException {
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/edens_car?user=root&password=admin")) {
            ...
            configuration.set(new CustomConnectionProvider())
                    .set(SQLDialect.MYSQL)
                    .set(DefaultExecuteListenerProvider.providers(new QueryRuntimeListener()));
            DSLContext context = DSL.using(configuration);
            List<Vehicle> vehicles = context.select(EDENS_CAR.VEHICLE.VEHICLE_ID, EDENS_CAR.VEHICLE.VEHICLE_COLOR, EDENS_CAR.VEHICLE.VEHICLE_CURR_PRICE)
                    .from(VEHICLE)
                    .where(VEHICLE.VEHICLE_MANUFACTURER.eq(val(2L)))
    ...
    public static void selectWithSubquery() throws SQLException {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/edens_car?user=root&password=admin")) {
            ...
            //table(context.select(vehicleId, max(lastPriceUpdate)).from(vehiclePriceChange).groupBy(vehicleId));
